Shader "Horus/Unlit/ItemStandard"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                o.normalUV = TRANSFORM_TEX(v.uv, _MainTex);
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float shininess = UNITY_ACCESS_INSTANCED_PROP(Props, _Shininess);

                float3 viewDir = normalize(i.worldViewDir);
                float3 tangentNormal = UnpackNormal(tex2D(_MainTex, i.normalUV));
