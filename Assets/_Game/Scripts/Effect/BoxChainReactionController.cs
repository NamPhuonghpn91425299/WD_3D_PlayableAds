...
public class BoxChainReactionController : SingletonBase<BoxChainReactionController>
{
    [SerializeField]
    ...
    public void InitializeBoxes()
    {
        ...
        boxes.Clear();
        GamePlayManager.Instance.CurrentQueueTargets.Clear();
        // Create initial boxes
        for (int i = 0; i < InitialBoxCount; i++)
        {
            CreateBox(true);
        }
        GamePlayManager.Instance.TotalQueueActiveCount = InitialBoxCount;
        CreateBox();
        RepositionAllBoxes(0);
    }

    public Transform CreateBox(bool isActive = false)
    {
        ...
        queueTarget.SetActive(isActive);
        GamePlayManager.Instance.CurrentQueueTargets.Add(queueTarget);
        return box;
    }
    ...
    public IEnumerator AddBoxWithAnimationCoroutine()
    {
        ...
        newBox.position = new Vector3((newPositions[boxes.Count - 2].x + newPositions[boxes.Count - 1].x) / 2, newPositions[boxes.Count - 1].y, newPositions[boxes.Count - 1].z);
        boxes[boxes.Count - 2].position = new Vector3(newPositions[boxes.Count - 2].x, newPositions[boxes.Count - 2].y + 0.2f, newPositions[boxes.Count - 2].z);
        for (var index = 0; index < boxes.Count; index++)
        {
            Transform box = boxes[index];
            // Reset position and scale of all boxes
            if (index == boxes.Count - 2)
            {
                box.DOMove(newPositions[index], 0.4f)
                   .SetEase(Ease.OutBack);
            }
            else box.DOMove(newPositions[index], 1f)
               .SetEase(Ease.OutElastic);
            box.localScale = originalScale;
        }
        yield break;
    ...
    private IEnumerator StartChainReactionCoroutine(List<Vector3> oldPositions, List<Vector3> newPositions)
    {
        ...
        {
            //if (soundData.GetAudioClip("booster1_impact") != null) GameAudioManager.Instance.PlayOneShot(soundData.GetAudioClip("booster1_impact"),soundData.GetSoundVolume("booster1_impact") );
            //SoundManager.Instance.PlayOneShot("booster1_impact");
            Debug.Log("Playing sound booster1_impact");
