...
public class DecoreControl : MonoBehaviour
{
    ...
    public void PulseOutOfParrentWool([Bridge.Ref] Vector3 forceSource, float forcevalue, float randomDirrectionFactor)
    {
        if(!Rigid) return;
        UseGravity(true);
        //transform.parent.parent -> to get the ObjectSpawner -_-
    ...
    public void SetColor([Bridge.Ref] Color color) { _color = color; }
