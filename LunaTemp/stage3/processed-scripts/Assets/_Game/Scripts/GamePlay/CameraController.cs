...
// Thêm các thuộc tính này vào khu vực PROPERTIES ở đầu class
    ...
    private void HandleTap([Bridge.Ref] Vector2 pos)
    {
    ...
    private WoolControl FindBestWoolNearRay([Bridge.Ref] Ray ray)
    {
    ...
    private void HandleHold([Bridge.Ref] Vector2 pos)
    {
    ...
    private Vector2 AdjustLunaMousePosition([Bridge.Ref] Vector2 originalPos)
    {
    ...
    private void HandleDrag([Bridge.Ref] Vector2 pos)
    {
        if (!_isActive) return;
    ...
    private void HandleDragSmoothly([Bridge.Ref] Vector2 pos)
    {
        if (!_isActive) return;
    ...
    /// <returns></returns>
    private bool IsVectorInRangeUpward([Bridge.Ref] Vector3 original, [Bridge.Ref] Vector3 target, float offset) { return original.y > target.y - offset && original.y < target.y + offset; }
